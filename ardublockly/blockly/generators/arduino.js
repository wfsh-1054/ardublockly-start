...
Blockly.Arduino.init = function (workspace) {
    ...
    Blockly.Arduino.defines_ = Object.create(null);
    // Create a dictionary of global definitions to be printed after variables
...
Blockly.Arduino.finish = function (code) {
    ...
    delete Blockly.Arduino.defines_;
 ...
 * Once a include is added it will not get overwritten with new code.
 * @param {!string} defineTag Identifier for this define code.
 * @param {!string} code Code to be define at the very top of the sketch.
 */
Blockly.Arduino.addDefine = function (defineTag, code) {
    if (Blockly.Arduino.defines_[defineTag] === undefined) {
        Blockly.Arduino.defines_[defineTag] = code;
    }
};
/**
 ...
 * @param {!string} preferredName Identifier for the function.
 * @param {!string} code Code to be included in the setup() function.
 * @return {!string} A unique function name based on input name.
 */
Blockly.Arduino.addFunction = function (preferredName, code) {
    if (Blockly.Arduino.codeFunctions_[preferredName] === undefined) {
        var uniqueName = Blockly.Arduino.variableDB_.getDistinctName(
            preferredName, Blockly.Generator.NAME_TYPE);
        Blockly.Arduino.codeFunctions_[preferredName] =
            code.replace(Blockly.Arduino.DEF_FUNC_NAME, uniqueName);
        Blockly.Arduino.functionNames_[preferredName] = uniqueName;
    }
    return Blockly.Arduino.functionNames_[preferredName];
};
...
/**
 * Encode a string as a properly escaped Arduino string, complete with single quotes.
 * @param {string} string Text to encode.
 * @return {string} Arduino string.
 * @private
 */
Blockly.Arduino.singlequote_ = function (string) {
