...
Blockly.StaticTyping.prototype.collectVarsWithTypes = function (workspace) {
    ...
    for (var i = 0; i < blocks.length; i++) {
        // add by darrin - 2021/11/24-start
        if (blocks[i].type.indexOf('procedures') > -1)
            continue;
        // add by darrin - 2021/11/24-end
...
Blockly.StaticTyping.prototype.setBlockTypeWarning =
    function (block, blockType, varName) {
        ...
        if ((blockType == Blockly.Types.CHILD_BLOCK_MISSING) ||
            ...
            block.setWarningText(Blockly.Msg.VARIABLES_WARN_TEXT.replace('%1', varName)
                .replace('%2', this.varTypeDict[varName].typeName)
                .replace('%3', blockType.typeName), warningLabel);
