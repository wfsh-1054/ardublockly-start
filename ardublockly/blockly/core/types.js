...
/** Byte number. */
Blockly.Types.BYTE = new Blockly.Type({
    typeId: 'Byte',
    typeMsgName: 'ARD_TYPE_BYTE',
    compatibleTypes: []
});
...
Blockly.Types.getValidTypeArray = function () {
    ...
    for (var typeKey in Blockly.Types) {
        if ((typeKey !== 'UNDEF') && (typeKey !== 'CHILD_BLOCK_MISSING') &&
            (typeKey !== 'NULL') && (typeKey !== 'ARRAY') && (typeKey !== 'FUNCTION') &&
...
/**
 * Get TypeID from type array by input type key
 * @param typeName
 * @return {Blockly.Types<String>} Blockly types.
 */
Blockly.Types.getTypeID = function (typeName) {
    var typeBlockly;
    for (var typeKey in Blockly.Types) {
        if (typeKey == typeName) {
            typeBlockly = Blockly.Types[typeKey];
            break;
        }
    }
    return typeBlockly;
