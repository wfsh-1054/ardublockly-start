...
Ardublockly.bindSettingsPathInputs = function () {
    ...
    Ardublockly.bindClick_(compilerEl, function () {
        showOpenDialog({
            ...
            if (files && files[0]) {
                /*Modify by darrin, for local path*/
                var projectRoot = projectLocator.getProjectRootPath();
                var file = files[0];
                if (file.indexOf(projectRoot) === 0)
                    file = file.replace(projectRoot + "\\", "");
                ArdublocklyServer.setCompilerLocation(file, function (jsonObj) {
    ...
    Ardublockly.bindClick_(sketchEl, function () {
        showOpenDialog({
            ...
            if (folders && folders[0]) {
                ...
                ArdublocklyServer.setSketchLocation(path, function (jsonObj) {
